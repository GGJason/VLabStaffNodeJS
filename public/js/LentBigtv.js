...
function analizeDate(){                                     //分析日期 與後端溝通
    ...
    if(startDate.value != '' && startTime.value != '' && endDate.value != '' && endTime.value != ''){
      document.getElementById('time_info').innerHTML =
        `<p><img class="loading" src="images/loading.gif" height="60" width="60">系統確認中，請稍候。</p>`;
      ...
      console.log(`./computer.php?calendar&check&start=${application.startDate} ${application.startTime}&end=${application.endDate} ${application.endTime}`);
      //時間分析 api接這裡
...
function recordINFO(){
  ...
  application.newuser = form.newuser.value;
...
function postMSG(){
  ...
  if(check()){
    //post method
    $.post('./form.php?workstation',application,function(msg){
      console.log(msg);
    });
